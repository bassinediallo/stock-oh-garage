...
<div class="container mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Modifier la Checklist du {{ \Carbon\Carbon::parse($checklist->date_verification)->format('d/m/Y') }}</h2>
    ...
    <form action="{{ route('checklists.update', $checklist) }}" method="POST">
        ...
        <div class="card mb-4">
            <div class="card-header">
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" id="date_verification" name="date_verification" 
                           value="{{ $checklist->date_verification }}" required>
                </div>
            </div>
        </div>
                            ...
                            @foreach ($options as $option)
                                <div class="form-check">
                                    <input type="radio" class="form-check-input" name="{{ $field }}" value="{{ $option }}"
                                        {{ $checklist->$field === $option ? 'checked' : '' }}>
                                    <label class="form-check-label">{{ $option }}</label>
                                </div>
                            @endforeach
                        </div>
                        <textarea class="form-control" name="{{ $field }}_remarques" 
                                  placeholder="Remarques">{{ $checklist->{$field.'_remarques'} }}</textarea>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="card mb-4">
            <div class="card-header bg-light">
                <h4>4. Contrôle final et observations générales</h4>
            </div>
            <div class="card-body">
                @foreach ([
                    ...
                    <div class="form-group mb-4">
                        ...
                        <div class="d-flex gap-3 mb-2">
                            @foreach ($options as $option)
                                <div class="form-check">
                                    <input type="radio" class="form-check-input" name="{{ $field }}" value="{{ $option }}"
                                        {{ $checklist->$field === $option ? 'checked' : '' }} required>
                                    <label class="form-check-label">{{ $option }}</label>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea class="form-control" name="materiel_manquant" rows="3" 
                              placeholder="Listez le matériel manquant ou à remplacer">{{ $checklist->materiel_manquant }}</textarea>
                </div>

                <div class="form-group mb-4">
                    <label>Recommandations et actions à entreprendre</label>
                    <textarea class="form-control" name="recommandations" rows="3" 
                              placeholder="Notez vos recommandations et les actions nécessaires">{{ $checklist->recommandations }}</textarea>
                </div>
            </div>
        </div>
